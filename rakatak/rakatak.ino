...
// ---Language for Menu and Portal -=- Thanks, @marivaaldo and @Mmatuda! ---
// #define LANGUAGE_EN_US //English (United States)
// #define LANGUAGE_PT_BR //Portuguese (Brazil)
// #define LANGUAGE_IT_IT //Italian
// #define LANGUAGE_FR_FR //French
// #define LANGUAGE_DX_DX //Daraĉrek
// #define LANGUAGE_RD_EN // reduced english (HUMOR ONLY, NOT USABLE)
...
uint16_t FGCOLOR=0xFFF1; // placeholder
  ...
  #define STICK_C_PLUS2
#endif
// If you don't specify what device you use, the code will automatically choose the STICKC PLUS2
#if !defined(LANGUAGE_EN_US) && !defined(LANGUAGE_PT_BR) && !defined(LANGUAGE_IT_IT) && !defined(LANGUAGE_FR_FR)
  #define LANGUAGE_EN_US
#endif
//If you don't specify your language, the code will automatically choose English-US

// -=-=- DEAUTHER -=-  @bmorcelli -=-=- | Discord: Pirata#5263 bmorcelli
#define DEAUTHER

#define BITMAP M5.Lcd.drawBitmap(0, 0, 240, 135, img)
...
QRCODE qrcodes[] = {   
  ...
  { "PIX", "https://oguser.xyz/redirect" },
}; //Add some new URLs here if you wanna troll your friends
...
int tunemenu_size = sizeof(tunemenu) / sizeof(MENU);

void tunemenu_setup() {
  cursor = 0;
  rstOverride = true;
  drawmenu(tunemenu, tunemenu_size);
  delay(500); // Prevent switching after menu loads up
}
...
void tunemenu_loop() {
  ...
  if (check_select_press()) {
    rstOverride = false;
    isSwitching = true;
    tunestate = tunemenu[cursor].command;
    #if defined(USE_EEPROM)
      EEPROM.write(6, tunestate);
      EEPROM.commit();
    #endif
    current_proc = 2;
  }
...
int pwrmenu_size = sizeof(pwrmenu) / sizeof(MENU);

void pwrmenu_setup() {
  cursor = 0;
  rstOverride = true;
  drawmenu(pwrmenu, pwrmenu_size);
  delay(500); // Prevent switching after menu loads up
}
...
void pwrmenu_loop() {
  ...
  if (check_select_press()) {
    rstOverride = false;
    isSwitching = true;
    //  if(pwrmenu[cursor].command == 1){
       //M5.Axp.PowerOff();
    //  }
      if(pwrmenu[cursor].command == 2){
        ESP.restart();
      }
    }
}
...
void setup() {
  ...
  if(check_next_press()){
    ...
    Serial.printf("EEPROM 6 - tunestate:  %d\n", EEPROM.read(6));
    if(EEPROM.read(0) > 3 || EEPROM.read(1) > 240 || EEPROM.read(2) > 100 || EEPROM.read(3) > 1 || EEPROM.read(4) > 19 || EEPROM.read(5) > 19 || EEPROM.read(6) > 1) {
      ...
      EEPROM.write(6, 1);    // Boot tunes ON
      EEPROM.commit();
    }
